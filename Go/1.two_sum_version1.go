//author    : liuzekun
//data      : 2016.05.22
//des       : Given an array of integers, return indices of the two numbers such that they add up to a specific target.
//summ      : leet code 1. Tow Sum
package main
import (
    "fmt"
)
...
func twoSum(nums []int, target int) []int {
    ...
    size := uint32((max_num - min_num) / 8 + 1)
    bitmap := make([]byte, size)
    for _, num := range nums {
        index, pos := (num - min_num) / 8, uint32((num - min_num) % 8)
        bitmap[index] |= (0x01 << pos)
    }
    
    res := []int{-1, -1}
    for nums_i, num := range nums {
        dffer := target - num
        if dffer < min_num || dffer > max_num {
            continue
        }
        index, pos := (dffer - min_num) / 8,  uint32((dffer - min_num) % 8)
        if bitmap[index] & (0x01 << pos) != 0 {
            for j := nums_i + 1; j < len(nums);  j++ {
                if dffer == nums[j] {
                    res[0], res[1] = nums_i, j
                    fmt.Printf("[%d, %d]\n", res[0], res[1])
                    return res
                }
            }
